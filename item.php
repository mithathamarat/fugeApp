...
foreach($result as $column){
	...
	@$names4 .= '$'. $column['Field'] . '='.'@$_POST['.'"'.$column['Field'].'"'.'];';
	
	@$form .= '
	'. $column['Field'] . ' = <?= $csql["'. $column['Field'] . '"] ?>
	';
...
<div class="container">
      <div class="header clearfix">
        ...
        <h3 class="text-muted">SELECT ITEM QUERY</h3>
      </div>
...
foreach($tables as $table){
  ...
  <?php  if(empty($names4)){}else{echo '<div class="form-group">
    <label >QUERY</label>
   <textarea class="form-control" rows="4">$csql = $db->prepare("SELECT * FROM '.$field.' WHERE '.current(explode(",", $names)).'=:uid");
$csql->execute(array(":uid"=>$'.current(explode(",", $names)).'));
$csql=$csql->fetch(PDO::FETCH_ASSOC); </textarea> </div> ';}  ?>    
  
      
<?php  if(empty($t)){}else{echo '<div class="form-group">
    <label >HTML</label>
   <textarea class="form-control" rows="8">'. $form . '</textarea> </div> ';}  ?>        
