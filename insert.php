...
<div class="container">
      <div class="header clearfix">
        ...
        <h3 class="text-muted">INSERT SQL</h3>
      </div>
...
<?php  if(empty($form)){}else{echo '<div class="form-group">
  ...
  <?php  if(empty($names4)){}else{echo '<div class="form-group">
    <label >$_POST</label>
   <textarea class="form-control" rows="8">'.explode(';', $names4,2)[1].'</textarea> </div> ';}  ?>    
      
      
<?php  if(empty($t)){}else{echo '<div class="form-group">
    <label >SQL</label>
   <textarea class="form-control" rows="8">$sqlekle = "INSERT INTO '.$field.' ('.  explode(',', $t,2)[1].') VALUES ('. explode(',', $t1,2)[1].')";
$q = $db->prepare($sqlekle);
$q->execute(array('.explode(',', $t2,2)[1].'));			  
if ($q === true) {echo "Kayıt yapılamadı sistem bir hata ile karşılaştı.";}
else {echo "ok";}</textarea> </div> ';}  ?>        
