...
<?php  if(empty($form)){}else{echo '<div class="form-group">
   ...
   <textarea class="form-control" rows="8"><form><div class="form-group">
  ...
  <?php  if(empty($names4)){}else{echo '<div class="form-group">
    <label >$_POST</label>
   <textarea class="form-control" rows="8">$'.current(explode(",", $names)).'=$_POST["'.current(explode(",", $names)).'"];</textarea> </div> ';}  ?>    
      
      
<?php  if(empty($t)){}else{echo '<div class="form-group">
    <label >SQL</label>
   <textarea class="form-control" rows="8">$query = $db->prepare("DELETE FROM '.$field.' WHERE '.current(explode(",", $names)).' = :'.current(explode(",", $names)).'");
